...
<template>
        ...
        <div class="p-fluid">
            <div class="formgrid grid">
                <div class="field col-12  md:col-4 mb-4">
                    ...
                    <Dropdown id="type" v-model="customer.type" :options="types" optionLabel="name" optionValue="id" :placeholder="$t('customers.account_type')" class="w-full" :disabled="saving"/>
                ...
                <div class="field col-12 md:col-4 mb-4" v-if="customer.type == 'person'">
                    ...
                    <InputText id="pesel" type="text" :placeholder="$t('customers.pesel')" class="w-full" v-model="customer.pesel" :disabled="saving" />
                </div>
    ...
    <div class="mb-4">
        <div class="flex justify-content-between align-items-center header-border-bottom pb-2 ">
            <h5 class="inline-flex  mb-0">{{ $t('customers.phone_list') }}</h5>
            <div class="text-right mb-0 inline-flex" >
                <Button icon="pi pi-phone" @click="addCustomerPhone" severity="secondary" v-tooltip.left="$t('customers.add_phone')" :label="$t('customers.add')" iconPos="right" class="text-center" size="small"></Button>
            </div>
        </div>
        <div class="mb-5 mt-3">
            <div class="overflow-auto">
                <table class="table w-full mt-2 mb-4" v-if="customer.contacts.phone.length">
                    ...
                    <tbody v-for="(phone, index) in customer.contacts.phone">
                        <tr>
                            <td class="text-right">
                                <Button type="button" @click="removeCustomerPhone(index)" severity="danger" :loading="saving" iconPos="right" icon="pi pi-trash" class="text-center mr-2"></Button>
                            </td>
                            <td>
                                <div class="flex">
                                    <div class="display-inline mr-3" style="min-width: 145px; max-width: 145px">
                                        <Dropdown id="type" :filterFields="phoneCodesFilterFields" filter v-model="customer.contacts.phone[index].prefix" :options="phoneCodes" optionValue="code" :placeholder="$t('customers.phone_code')" class="w-full" :disabled="saving">
                                    ...
                                    <div class="w-full" style="min-width: 150px">
                                        <InputMask mask="999-999-999?999999" slotChar="" :class="{'p-invalid' : v.customer.contacts.phone[index].$error}" :placeholder="$t('customers.phone')" :value="phone.val" class="w-full" v-model="customer.contacts.phone[index].val"  />
                                    </div>
                                </div>
                            </td>
        ...
        <div class="flex justify-content-between align-items-center header-border-bottom pb-2 mb-5">
            <h5 class="inline-flex  mb-0">Adresy e-mail</h5>
            <div class="text-right mb-0 inline-flex" >
                <Button icon="pi pi-at" @click="addCustomerEmail" severity="secondary" v-tooltip.left="$t('customers.add_email')" :label="$t('customers.add')" iconPos="right" class="text-center" size="small"></Button>
            </div>
        </div>
        <div class="mb-5 mt-3">
            <div class="overflow-auto">
                <table class="table w-full mt-2 mb-4" v-if="customer.contacts.email.length">
                    ...
                    <tbody v-for="(email, index) in customer.contacts.email">
                        <tr>
                            <td class="text-right">
                                <Button type="button" @click="removeCustomerEmail(index)" severity="danger" :loading="saving" iconPos="right" icon="pi pi-trash" class="text-center mr-2"></Button>
                            </td>
                            <td>
                                <InputText type="text" :placeholder="$t('customers.email')" :class="{'p-invalid' : v.customer.contacts.email[index].$error}" :value="email.val" class="w-full" v-model="customer.contacts.email[index].val"  />
                            </td>
                            <td class="text-center align-middle">
